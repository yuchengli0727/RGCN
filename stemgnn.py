...
# 输入维度 [batch squence/window_size/time_step, nodes] -- [32 12 140]
...
class StockBlockLayer(nn.Module):
    ...
    def forward(self, x, mul_L):
        ...
        x = x.unsqueeze(1)
        gfted = torch.matmul(mul_L, x)      # D A D
...
class Model(nn.Module):
    def __init__(self, units, stack_cnt, time_step, multi_layer, horizon=1, dropout_rate=0.5, leaky_rate=0.2,
        ...
        self.unit = units   # 输入的个数/节点个数 307
        self.stack_cnt = stack_cnt  # stemblock的个数，论文中是两个，我只需要一个因此不用管
